...
#define LOGI(...) ((void)__android_log_print(ANDROID_LOG_INFO, "mine", __VA_ARGS__))
...
static GLuint compileShader(GLenum type, const char *shaderName, const char *shaderString) {
...
char* simple_vert =    "attribute highp vec3 a_position;\n"
...
char* simple_frag = "varying lowp vec3 v_color;\n"
                    "\n"
                    "void main() {\n"
...
static void onFrame(GLFMDisplay *display, double frameTime) {
    ...
    // Draw triangle
    if (app->program == 0) {
        GLuint vertShader = compileShader(GL_VERTEX_SHADER, "simple.vert", simple_vert);
        GLuint fragShader = compileShader(GL_FRAGMENT_SHADER, "simple.frag", simple_frag);
...
#include "glfm_platform_android.c"